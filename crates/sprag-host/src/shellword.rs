...
//!
//! Both consumers use it for the same reason — a string is about to be PASTED at a shell prompt as a
//! command line, so every word must survive word-splitting and expansion exactly as written.
...
/// Quote `word` for a shell command line: returned as-is when every character is
